...
@export var total_challenges: int
#user_sav_path
#user://UserSav.tres
...
func get_diamond_count(sav_slot: int) -> int:
	var sav: GameSaver = get_game_sav(sav_slot)
	var diamond_count: int
	for level_count in sav.level_sav:
		var current_level_sav = sav.level_sav[level_count]
		diamond_count += current_level_sav[2]
	return diamond_count


func get_bedrock_count(sav_slot: int) -> int:
	var sav: GameSaver = get_game_sav(sav_slot)
	var bedrock_count: int
	for level_count in sav.level_sav:
		var current_level_sav = sav.level_sav[level_count]
		bedrock_count += current_level_sav[3]
