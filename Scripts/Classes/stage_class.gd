...
signal on_prepare_music_playing
signal on_battle_music_playing
signal on_boss_music_playing
signal on_prepare_music_stop
signal on_battle_music_stop
signal on_boss_music_stop
...
@onready var music_node: Node = $Musics
...
@onready var dead_body_node: Node2D = $Background/DeadBodyNode
@onready var danger_tip_layer: Control = $StageUI/DangerTipLayer
...
func add_hero_skill_button():
	...
	match stage_sav.select_hero_id:
		...
		10: hero_skill_button = load("res://Scenes/UI-Components/Stages/SkillButton/rosa_skill_button.tscn").instantiate()
		5: hero_skill_button = load("res://Scenes/UI-Components/Stages/SkillButton/kong_shu_lin_skill_button.tscn").instantiate()
		6: hero_skill_button = load("res://Scenes/UI-Components/Stages/SkillButton/golden_diamond_skill_button.tscn").instantiate()
		11: hero_skill_button = load("res://Scenes/UI-Components/Stages/SkillButton/steve_sp_skill_button.tscn").instantiate()
...
func switch_music_play_mode(music_id: int):
	...
	if music_id == 0: 
		...
		on_prepare_music_playing.emit()
	else: 
		preparation_music.stop()
		on_prepare_music_stop.emit()
	...
	if music_id == 1: 
		...
		battle_music.stop()
		on_battle_music_stop.emit()
	
	if music_id == 2: 
		boss_music.play()
		on_boss_music_playing.emit()
	else: 
		boss_music.stop()
		on_boss_music_stop.emit()
	pass
...
func summon_hero(summon_pos:Vector2 = hero_summon_marker.position):
	...
	match stage_sav.select_hero_id:
		...
		5:
			hero = load("res://Scenes/Allys/Heroes/kong_shu_lin.tscn").instantiate()
		6:
			hero = load("res://Scenes/Allys/Heroes/golden_diamond.tscn").instantiate()
		7:
		...
		10:
			hero = load("res://Scenes/Allys/Heroes/rosa.tscn").instantiate()
		11:
			hero = load("res://Scenes/Allys/Heroes/steve_sp.tscn").instantiate()
...
func wave_money_get(wave_count: int):
	...
	if OS.get_name() == "Android":
		...
		var direction: Vector2 = Stage.instance.stage_camera.get_local_mouse_position()
		pos += direction / Stage.instance.stage_camera.zoom
		money_effect.position = pos
...
func get_closest_main_enemy_path(target_pos: Vector2) -> EnemyPath:
	if stage_count == 1: return get_closest_enemy_path(target_pos)
	
	var path_list: Array[EnemyPath]
	for i in int(enemies.get_child_count() / 3):
		var enemy_path: EnemyPath = enemies.get_child(3 * i)
		if enemy_path.enabled: path_list.append(enemy_path)
