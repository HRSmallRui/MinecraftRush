...
func _ready() -> void:
	...
	match move_type:
		...
		MoveType.Straight:
			...
			move_tween.tween_property(self,"global_position",target_position,time)
			move_tween.finished.connect(func():straight_move_finished.emit())
...
func enemy_take_damage(enemy: Enemy):
	#print(damage)
	var origin_health: int = enemy.current_data.health
	...
	delta_health = origin_health - enemy.current_data.health
	delta_health = maxi(delta_health,0)
	...
	if source is ArcherTower and level >= 5:
		if randf_range(0,1) < 0.1 and delta_health > 0:
			if enemy.enemy_state != Enemy.EnemyState.DIE:
				...
				blood_buff.dot_damage[0] = int(float(delta_health) * 0.8)
				#print(delta_health)
