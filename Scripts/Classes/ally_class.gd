...
@onready var intercepting_shape: CollisionShape2D = $UnitBody/InterceptingArea/InterceptingShape
...
func translate_to_new_state(new_state: AllyState):
	match new_state:
		AllyState.IDLE:
			...
			intercepting_shape.disabled = false
			#change_collision_layer(false)
			body_collision.disabled = false
			hurt_box.monitorable = true
		AllyState.MOVE:
			#intercepting_shape.disabled = true
...
func rebirth():
	...
	if is_fly:
		unit_body.set_collision_layer_value(5,true)
		hurt_box.set_collision_layer_value(5,true)
	else:
		unit_body.set_collision_layer_value(4,true)
...
func take_damage(damage: int, damage_type: DataProcess.DamageType, broken_rate: float, far_attack: bool = false, source: Node2D = null,explosion: bool = false,aoe_attack: bool = false, deadly: bool = true) -> bool:
	if ally_state == AllyState.DIE: return false
