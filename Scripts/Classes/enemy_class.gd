...
@onready var screen_notifier: VisibleOnScreenNotifier2D = $UnitBody/ScreenNotifier
...
func battle_process():
	current_intercepting_units.erase(null)
	...
	if current_intercepting_units.size() > 0:
		if current_intercepting_units[0] == null: current_intercepting_units.remove_at(0)
		if current_intercepting_units.is_empty(): return
...
func translate_to_new_state(new_state: EnemyState):
	match new_state:
		...
		EnemyState.DIE:
			...
			if Stage.instance.information_bar.current_check_member == self: Stage.instance.ui_process(null)
			for ally in current_intercepting_units:
				if current_intercepting_units == null: continue
				ally.current_intercepting_enemy = null
				if ally.ally_state == Ally.AllyState.MOVE: ally.move_back()
			current_intercepting_units.clear()
			if danger_tip != null:
				danger_tip.queue_free()
...
func die_explosion_process():
	...
	Stage.instance.dead_body_node.add_child(dead_explosion)
	pass
...
func take_damage(damage: int, damage_type: DataProcess.DamageType, broken_rate: float, far_attack: bool = false, source: Node2D = null,explosion: bool = false,aoe_attack: bool = false, deadly: bool = true) -> bool:
	if enemy_state == EnemyState.DIE: return false
...
func danger_tip_process():
	if screen_notifier.is_on_screen() and danger_tip != null:
		danger_tip.queue_free()
		danger_tip = null
	if !screen_notifier.is_on_screen() and danger_tip == null and progress_ratio > 0.6:
		var new_danger_tip: DangerTip = preload("res://Scenes/UI-Components/Stages/danger_tip.tscn").instantiate()
		danger_tip = new_danger_tip
		danger_tip.linked_enemy = self
		Stage.instance.danger_tip_layer.add_child(danger_tip)
	pass
