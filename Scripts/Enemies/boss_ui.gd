...
func _process(delta: float) -> void:
	if health_tween != null:
		health_tween.kill()
	health_tween = create_tween().set_speed_scale(1 / Engine.time_scale)
	health_tween.tween_property(health_bar,"value",linked_boss.health_bar.value,abs(linked_boss.health_bar.value - health_bar.value)/ 1)
	pass
