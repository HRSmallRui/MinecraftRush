...
func _process(delta: float) -> void:
	...
	if poise_tween != null: poise_tween.kill()
	poise_tween = create_tween().set_speed_scale(1 / Engine.time_scale)
	var target_value: float = float(poise) / max_poise
	poise_tween.tween_property(poise_bar,"value",target_value ,abs(target_value - poise_bar.value)/ 1 )
...
func take_damage(damage: int, damage_type: DataProcess.DamageType, broken_rate: float, far_attack: bool = false, source: Node2D = null,explosion: bool = false,aoe_attack: bool = false, deadly: bool = true) -> bool:
	if invincible or enemy_state == EnemyState.DIE:
		return false
