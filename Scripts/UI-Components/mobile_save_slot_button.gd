...
func _ready() -> void:
	...
	if linked_sav_slot != null:
		...
		diamond_label.text = str(Global.get_diamond_count(slot_number)) + " / " + str(Global.total_challenges)
		bedrock_label.text = str(Global.get_bedrock_count(slot_number)) + " / " + str(Global.total_challenges)
	pass
