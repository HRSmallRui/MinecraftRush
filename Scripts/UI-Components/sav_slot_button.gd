...
func show_old(sav: GameSaver):
	...
	total_diamonds = Global.get_diamond_count(slot_number)
	total_bedrocks = Global.get_bedrock_count(slot_number)
	last_sav_time = sav.last_sav_time
	
	$PastSav/LabelList/StarLabel.text = str(total_stars) + " / " + str(Global.total_stars)
	$PastSav/LabelList/DiamondLabel.text = str(total_diamonds) + " / " + str(Global.total_challenges)
	$PastSav/LabelList/BedrockLabel.text = str(total_bedrocks) + " / " + str(Global.total_challenges)
	pass
