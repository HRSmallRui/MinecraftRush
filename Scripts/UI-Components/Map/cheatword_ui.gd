...
func ROSA(label: Label):
	label.text = "已成功解锁蔷薇"
	if sav.hero_sav[10].unlocked:
		label.text = "该英雄已解锁"
	else:
		sav.hero_sav[10].unlocked = true
	pass


func KONGSHULIN(label: Label):
	label.text = "已成功解锁空树林"
	if sav.hero_sav[5].unlocked:
		label.text = "该英雄已解锁"
	else:
		sav.hero_sav[5].unlocked = true
	pass


func GOLDENDIAMOND(label: Label):
	label.text = "已成功解锁金钻"
	if sav.hero_sav[6].unlocked:
		label.text = "该英雄已解锁"
	else:
		sav.hero_sav[6].unlocked = true
	pass
