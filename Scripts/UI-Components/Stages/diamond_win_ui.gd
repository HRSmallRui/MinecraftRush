...
func save_reward():
	var key:String = str(Stage.instance.stage_count) + "_diamond"
	...
	if stage_block[2] > 0: return
	if stage_block[3] == 0:
		Map.level_animation_type = Map.LevelAnimationType.Campaign_Diamond
	else:
		Map.level_animation_type = Map.LevelAnimationType.Bedrock_Diamond
	Map.animation_stage_count = Stage.instance.stage_count
	
	Stage.instance.stage_sav.level_sav[Stage.instance.stage_count][2] = 1
	Stage.instance.stage_sav.total_stars += 1
	Stage.instance.stage_sav.can_use_stars += 1
	Stage.instance.stage_sav.total_diamonds += 1
