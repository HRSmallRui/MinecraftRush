...
@export var process_time_scale: float = 1
@export var waiting_time_total_scale: float = 1
...
func subtitle_change(new_word: String, appear_time: float, text_color: Color):
	await get_tree().create_timer(appear_time * waiting_time_total_scale,process_mode == ProcessMode.PROCESS_MODE_ALWAYS).timeout
	subtitle.modulate = text_color
	var word_tween: Tween = create_tween()
	subtitle.text = new_word
	subtitle.visible_characters = 0
	word_tween.tween_property(subtitle,"visible_characters",new_word.length(),float(new_word.length()) / 30 * process_time_scale)
...
func cg_play(cg_texture: Texture, cg_appear_time: float, cg_during_time: float):
	await get_tree().create_timer(cg_appear_time * waiting_time_total_scale,process_mode == ProcessMode.PROCESS_MODE_ALWAYS).timeout
	cg_player.modulate.a = 0
	cg_player.texture = cg_texture
	create_tween().tween_property(cg_player,"modulate:a",1,0.5)
	await get_tree().create_timer(cg_during_time * waiting_time_total_scale,process_mode == ProcessMode.PROCESS_MODE_ALWAYS).timeout
